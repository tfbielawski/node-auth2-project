...
const restricted = (req, res, next) => {
  ...
  //verify token, secret, and options callback to handle result
  else{
...
const only = role_name => (req, res, next) => {
  if(role_name === req.decodedToken.role_name){next()}
...
const checkUsernameExists = async (req, res, next) => {
  try{
    ...
    //If the user doesn't exist (the array has no length)
...
const validateRoleName = (req, res, next) => {
  ...
  else if(req.body.role_name.trim() === "admin"){ next({ status: 422, message: "Role name can not be admin"})}
  else if (req.body.role_name.trim().length > 32){ next({ status: 422, message: 'Role name can not be longer than 32 chars'})}
  else {
